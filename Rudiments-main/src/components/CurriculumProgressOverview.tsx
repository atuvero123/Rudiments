...
export const CurriculumProgressOverview: React.FC<{ skills: GranularSkill[] }> = ({ skills }) => {
  ...
  const verificationTransport = deriveCanonicalVerificationTransport(activeComp, findTeachingDefinition(activeComp.id));
  ...
  return (
    <div className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm sm:p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <div className="rounded-xl border border-stone-200 bg-stone-50 p-3"><span className="block text-[9px] font-black uppercase text-stone-400">Formal Test</span><strong className="text-sm text-stone-950">{verificationTransport.bpm} BPM / {formatVerificationLength(verificationTransport)}</strong></div>
      </div>
