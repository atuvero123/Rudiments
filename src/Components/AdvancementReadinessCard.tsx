...
export const AdvancementReadinessCard: React.FC<AdvancementReadinessCardProps> = ({
  ...
  const verificationTransport = deriveCanonicalVerificationTransport(competency, findTeachingDefinition(competency.id));
  ...
  return (
    <div className={`rounded-2xl border p-4 ${stateClasses[readiness.state]} ${compact ? 'space-y-3' : 'space-y-4'}`}>
      ...
      <div className="grid grid-cols-2 gap-2 text-[11px] sm:grid-cols-4">
        ...
        <div className="rounded-xl border border-current/10 bg-white/60 p-2.5"><span className="block text-[9px] font-black uppercase opacity-60">{verificationTransport.completionMode === 'BARS' ? 'Test Length' : 'Test Duration'}</span><strong>{formatVerificationLength(verificationTransport)}</strong></div>
