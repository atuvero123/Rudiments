...
export const CompetencyVerificationModal: React.FC<CompetencyVerificationModalProps> = ({
  ...
  const currentVerificationBar = isBarGoverned && requiredBars
    ? Math.min(requiredBars, Math.floor(Math.max(0, verificationBeatTicks - 1) / transportPulsesPerBar) + 1)
    : 1;
  const currentNotationBar = Math.min(notationRequiredBars, currentVerificationBar);
  const barProgress = requiredPulseCount
    ? Math.min(100, (verificationBeatTicks / requiredPulseCount) * 100)
    : 0;
      ...
      <div className={`my-8 w-full ${isNotationVerification && state === 'PLAYING' ? 'max-w-3xl' : 'max-w-xl'} space-y-5 rounded-3xl border border-stone-200 bg-white p-5 shadow-2xl sm:p-7`}>
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          ...
          <div className="rounded-xl border border-stone-200 bg-stone-50 p-3">
            <span className="block text-[9px] font-black uppercase text-stone-400">{isNotationVerification ? 'Chart' : isBarGoverned ? 'Length' : 'Duration'}</span>
            ...
            {isBarGoverned && <span className="mt-0.5 block text-[9px] font-bold text-stone-400">≈ {formatVerificationSeconds(verificationTransport.durationSeconds)}s</span>}
          </div>
        ...
        {state === 'COUNT_IN' && (
          <div className="rounded-3xl border border-amber-200 bg-amber-50 py-9 text-center">
            ...
            <p className="mt-2 text-xs text-stone-500">{isNotationVerification ? 'The chart appears after the count-in.' : isBarGoverned ? 'The governed bar run starts after the count-in.' : 'The verification clock starts after the count-in.'}</p>
          </div>
        )}
        ...
        {state === 'PLAYING' && (
          <div className="space-y-4 rounded-3xl bg-stone-950 p-5 text-white">
            <div className="flex items-center justify-between gap-3 text-xs">
              ...
              {isNotationVerification ? (
                <span className="font-mono text-stone-300">{notationRequiredBars}-BAR SIGHT-READ</span>
              ) : isBarGoverned && requiredBars ? (
            ...
            {!isNotationVerification && (
              <div className="h-2 overflow-hidden rounded-full bg-stone-800">
                <div
                  ...
                  style={{ width: `${isBarGoverned ? barProgress : Math.min(100, (elapsedSeconds / verificationTransport.durationSeconds) * 100)}%` }}
                />
              </div>
            )}
