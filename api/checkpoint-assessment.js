...
export default async function handler(req,res){
  ...
  try{const {track,currentLevel,targetLevel}=readBody(req); const prompt='Design a supportive qualitative Checkpoint Assessment explanation for the "'+(track||'Rudiments')+'" skill track from '+(currentLevel||'Beginner')+' toward '+(targetLevel||'Intermediate')+'. The app deterministic curriculum remains authoritative; provide 3-4 concrete self-check criteria, 2 practical tests, and next steps without claiming to certify the student yourself.'; const assessment=await generate(prompt); return res.status(200).json({apiVersion:API_VERSION,assessment});}
  catch(err){console.error('Checkpoint API Error:',err);return res.status(Number.isFinite(err?.status)?err.status:500).json({error:err?.message||'Unexpected server error.',code:err?.code||'SERVER_ERROR',apiVersion:API_VERSION});}
}
